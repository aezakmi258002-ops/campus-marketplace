...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 dark:bg-gray-900 dark:text-gray-100 transition-colors duration-300 overflow-x-hidden">
      {/* Navbar */}
      ...
      {/* Hero Banner Section */}
      <section className="relative bg-gradient-to-br from-blue-700 via-indigo-800 to-purple-900 dark:from-gray-900 dark:via-indigo-950 dark:to-slate-900 text-white py-16 px-4 text-center overflow-hidden">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {mockProducts.map((product) => (
            <div
              key={product.id}
              className="group bg-white dark:bg-gray-800 rounded-2xl border border-gray-200/80 dark:border-gray-700/80 overflow-hidden flex flex-col cursor-pointer transition-all duration-300 hover:shadow-xl hover:border-blue-400/50"
            >
              {/* 🖼️ เมื่อเอาเมาส์ชี้ ภาพสินค้าจะขยายและลอยขึ้นเนียนๆ */}
              <div className="h-48 bg-gray-100 dark:bg-gray-700 relative overflow-hidden">
                <img
                  src={product.image}
                  alt={product.title}
                  className="w-full h-full object-cover transition-transform duration-500 ease-out group-hover:scale-110 group-hover:-translate-y-1.5"
                />
                <span className="absolute top-2 right-2 bg-black/60 text-white text-[10px] px-2 py-1 rounded-md backdrop-blur-sm z-10">
                  {product.category}
                </span>
                <span className="absolute bottom-2 left-2 bg-blue-600/90 text-white text-[10px] px-2 py-0.5 rounded-full backdrop-blur-sm font-medium z-10">
                  📍 {product.location}
                </span>
              </div>

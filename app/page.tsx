...
export default function Home() {
  ...
  // ทำงานเมื่อค่า isDarkMode เปลี่ยนแปลง
  useEffect(() => {
    console.log('🔄 [Effect Triggered] isDarkMode =', isDarkMode);
    
    if (isDarkMode) {
      document.documentElement.classList.add('dark');
      console.log('✅ Added "dark" class to <html>:', document.documentElement.className);
    } else {
      document.documentElement.classList.remove('dark');
      console.log('❌ Removed "dark" class from <html>:', document.documentElement.className);
    }
  }, [isDarkMode]);

  const toggleDarkMode = () => {
    console.log('👆 [Button Clicked] Switching mode...');
      ...
      <header className="sticky top-0 z-50 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* ปุ่มสลับ Light / Dark Mode พร้อม console.log */}
            <button
              onClick={toggleDarkMode}
              type="button"
              className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-xl border border-gray-300 dark:border-gray-600 cursor-pointer"
